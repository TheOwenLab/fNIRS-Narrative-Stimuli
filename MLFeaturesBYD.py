# -*- coding: utf-8 -*-
"""
Created on Fri Dec 16 12:29:53 2022
...
# load group mask
gMask = loadmat(os.path.join(path,"BYDvScramMaskFDR.mat"))["BYDMask"].T.reshape(-1,) 
...
sigMask = loadmat("BYDMaskML.mat")['BYDMask'].astype("bool")
...
gAcc = np.ones((nSub,nIter,nChan)) * np.nan
    ...
    for ii in range(nIter):
        for cc in range(0,nChan):
            ...
            if gPred.shape[1] < 2:
                ...
                gPred = gPred[[0,1]]
                # save to array
                gProb[ss,ii,y_test.astype("int"),:,cc] = gPred
...
savemat("BYDSingleChannelRes.mat",{"pConf":pConf, "pAcc":pAcc})