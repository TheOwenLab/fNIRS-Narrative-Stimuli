# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 16:41:35 2023
...
# load in ISCs and Hold Out ISCs 
...
# load Hold Out masks from MATLAB
TKNMask = loadmat("GroupFDRML.mat")['GroupFDR'][2,:,:].astype("bool") # Taken Intact
TKNSMask = loadmat("GroupFDRML.mat")['GroupFDR'][3,:,:].astype("bool") # Taken Scrambled
...
# useful helpful function to convert the outputs of fmin
...
IMPORTANCE = np.zeros((len(ESTIMATORS),nSub)); VALIDATION_PERFORMANCE = []; 
...
# save results 
np.save("TakenPrediction.npy",GROUPPREDICTION)
...
AccNull = np.load("TakenAccPerm.npy")
np.sum(groupProb < np.mean(AccNull,1))

PreNull = np.load("TakenPrecisionPerm.npy")
np.sum(groupPre < np.mean(PreNull,1))


RecNull = np.load("TakenRecallPerm.npy")
...
fig.suptitle("Taken vs. Taken Scrambled",fontsize = 10)
...
plt.xticks([0.5,1.5], labels = ["Taken Scrambled", "Taken"])
plt.yticks([0.5,1.5], labels = ["Taken Scrambled", "Taken"])
plt.xlabel("Predicted Condition"); plt.ylabel("Actual Condition")
#plt.title("Taken vs Taken Scrambled")
plt.show()
