# -*- coding: utf-8 -*-
"""
Created on Fri Nov 24 17:35:45 2023
...
TKNMask = loadmat("GroupFDRML.mat")['GroupFDR'][2,:,:].astype("bool")
TKNSMask = loadmat("GroupFDRML.mat")['GroupFDR'][3,:,:].astype("bool")

# alternatively, use IgS mask
IgSMask = loadmat("FDRGroupBinIgS.mat")['FDRGroupBinIgS'][1,:,:].astype("bool")
...
# use no mask
#FDRMask = np.ones(IgSMask.shape) == 1
...
sns.catplot(data=df[mIdx.reshape(-1,)==1], x = "Condition", y = "ISC",hue = "Subject",kind = "point")
plt.show()
...
GROUPPREDICTION = np.zeros((nSub,nCond,nChan)); NCOMPONENTS = []; VOTEPREDICTION = np.zeros((nSub,nCond));
VOTEPREDICTIONPROBA = np.zeros((nSub,nCond));
...
savemat("TakenSingleChannelResVote.mat",{"pAcc":pAcc})